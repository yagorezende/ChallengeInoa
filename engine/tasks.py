    ...
    if tip:
        ...
        send_email_task.delay(
            ...
            monitor_data={
                ...
                'stock_price': stock_price.id,
                'user_name': monitor.user.get_full_name()
            }
        )
    return None
...
def send_email_task(subject, message, from_email, recipient_list, monitor_data):
    """
    Task to send email
    ...
    :param monitor_data: data from the monitor as a dict
    :return: None
    """
    ...
    if status: # status == number of emails sent -> if all successful, status == len(recipient_list)
        ...
        monitor = UserMonitorStock.objects.get(id=monitor_data.get('monitor'))
